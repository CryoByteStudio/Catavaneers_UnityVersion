...
namespace AI
{
    public class CatAIController : AIController
    {
        ...
        {
            // add states
            finiteStateMachine.AddState(AIState.FindTarget.ToString(), new CatFindTarget(this));
            finiteStateMachine.AddState(AIState.Chase.ToString(), new CatChase(this));
            finiteStateMachine.AddState(AIState.Attack.ToString(), new CatAttack(this));
            finiteStateMachine.AddState(AIState.Frenzy.ToString(), new CatFrenzy(this));
            finiteStateMachine.AddState(AIState.PreAttack.ToString(), new CatPreAttack(this));
            finiteStateMachine.AddState(AIState.Death.ToString(), new CatDeath(this));
        ...
        private static List<HealthComp> GetAllTargets()
        {
            ...
            {
                if (allTargetsWithHealthComponent[i].myClass == CharacterClass.Caravan
                 || allTargetsWithHealthComponent[i].myClass == CharacterClass.Player)
                {
